"""
Image loading end-to-end test: FileBrowser → FilePresenter → AppController → Store
...
class ImageLoadingTest(QWidget):
    ...
    def __init__(self):
        ...
        # ── Signal loggers ──────────────────────────
        panel_logger = SignalLogger(self.log_window, "FileBrowser")
        panel_logger.connect_signal(self.file_browser.fileDoubleClicked, "fileDoubleClicked")
        panel_logger.connect_signal(self.file_browser.pathChanged, "pathChanged")
        ...
        self.log_window.add_log("Chain wired: FileBrowser → FilePresenter → Controller → Store")
